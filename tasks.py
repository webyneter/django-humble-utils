...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-humble-utils runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-humble-utils.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_humble_utils")
...
def lint(c):
    ...
    """
    c.run("flake8 django-humble-utils tests")
...
def release(c, bumpsize=''):
    ...
    import django_humble_utils
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_humble_utils.__version__))
    c.run("git push --tags")
